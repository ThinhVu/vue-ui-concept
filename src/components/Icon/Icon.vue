...
function parseIconData(iconData) {
  const iconPath = {
    icon: '',
    size: '',
    color: ''
...
export default {
  ...
  setup(props, ctx) {
    return () => {
      ...
      const {icon, size, color} = parseIconData(iconData)
      const iconClass = [ icon, color.startsWith('.') && color.substr(1) ]
